...
**1. API 설명**
연주 중 마디마다 **측정 입력(마이크 오디오·카메라 프레임)** 을 보내면, 백엔드가 직접 측정(`SwiftF0`/`onset 앵커 매칭(librosa)`/`MediaPipe`)해 도메인별 state를 만들고 멀티에이전트가 피드백을 돌려주는 WebSocket 채널. 측정·분석·피드백·보상 모두 마디 1개 단위이며 백엔드에서 처리한다. 연결 전제: 세션 생성(REST) → 권한 허용·카운트인(클라) 완료 후 연결.
